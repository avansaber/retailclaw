...
def update_price_list(conn, args):
    ...
    if price_list_type is not None:
        _validate_enum(price_list_type, VALID_PRICE_LIST_TYPES, "price-list-type")
        updates.append("price_list_type = ?")
        params.append(price_list_type)
    ...
    if price_list_status is not None:
        ...
        params.append(price_list_status)
    ...
    params.append(pl_id)
    conn.execute(f"UPDATE retailclaw_price_list SET {', '.join(updates)} WHERE id = ?", params)
...
def list_price_lists(conn, args):
    ...
    total = conn.execute(f"SELECT COUNT(*) FROM retailclaw_price_list WHERE {where_sql}", params).fetchone()[0]
    params.extend([args.limit, args.offset])
    rows = conn.execute(
        f"SELECT * FROM retailclaw_price_list WHERE {where_sql} ORDER BY created_at DESC LIMIT ? OFFSET ?",
        params
    ).fetchall()
    ok({
...
def update_price_list_item(conn, args):
    ...
    if rate is not None:
        updates.append("rate = ?")
        params.append(str(round_currency(to_decimal(rate))))
        changed.append("rate")

    min_qty = getattr(args, "min_qty", None)
    if min_qty is not None:
        updates.append("min_qty = ?")
        params.append(str(to_decimal(min_qty)))
    ...
    updates.append("updated_at = datetime('now')")
    params.append(pli_id)
    conn.execute(f"UPDATE retailclaw_price_list_item SET {', '.join(updates)} WHERE id = ?", params)
...
def list_price_list_items(conn, args):
    ...
    where_sql = " AND ".join(where)
    total = conn.execute(f"SELECT COUNT(*) FROM retailclaw_price_list_item WHERE {where_sql}", params).fetchone()[0]
    params.extend([args.limit, args.offset])
    rows = conn.execute(
        f"SELECT * FROM retailclaw_price_list_item WHERE {where_sql} ORDER BY created_at DESC LIMIT ? OFFSET ?",
        params
    ).fetchall()
    ok({
        "rows": [row_to_dict(r) for r in rows],
...
def update_promotion(conn, args):
    ...
    if discount_value is not None:
        ...
        params.append(str(round_currency(to_decimal(discount_value))))
    ...
    if min_purchase is not None:
        ...
        params.append(str(round_currency(to_decimal(min_purchase))))
    ...
    conn.execute(f"UPDATE retailclaw_promotion SET {', '.join(updates)} WHERE id = ?", params)
...
def list_promotions(conn, args):
    ...
    where_sql = " AND ".join(where)
    total = conn.execute(f"SELECT COUNT(*) FROM retailclaw_promotion WHERE {where_sql}", params).fetchone()[0]
    params.extend([args.limit, args.offset])
    rows = conn.execute(
        f"SELECT * FROM retailclaw_promotion WHERE {where_sql} ORDER BY created_at DESC LIMIT ? OFFSET ?",
        params
    ).fetchall()
    ok({
        "rows": [row_to_dict(r) for r in rows],
...
def activate_promotion(conn, args):
    ...
    conn.execute(
        "UPDATE retailclaw_promotion SET promo_status = 'active', updated_at = datetime('now') WHERE id = ?",
        (promo_id,)
    )
...
def deactivate_promotion(conn, args):
    ...
    conn.execute(
        "UPDATE retailclaw_promotion SET promo_status = 'paused', updated_at = datetime('now') WHERE id = ?",
        (promo_id,)
    )
